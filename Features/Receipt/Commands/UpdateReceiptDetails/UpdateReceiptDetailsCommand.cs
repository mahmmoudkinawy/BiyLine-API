namespace BiyLineApi.Features.Receipt.Commands.UpdateReceiptDetails
{
            ...
            public async Task<Result<Response>> Handle(Request request, CancellationToken cancellationToken)
            {
                using (var trans = await _context.Database.BeginTransactionAsync())
                {
                    try
                    {
                        ...
                        var detail = await _context.ReceiptDetails.FirstOrDefaultAsync(d => d.Id == request.Id, cancellationToken: cancellationToken);
                        if (detail == null)
                        {
                        ...
                        var warehouseLog = await _context.WarehouseLogs.FirstOrDefaultAsync(W => W.DocumentId == detail.ReceiptId && W.DocumentType == DocumentType.Receipt);
